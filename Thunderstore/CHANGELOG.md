...
- Moved InventorySlots YAML files into `BepInEx/config/InventorySlots/`: server-authoritative slot/group rules live in `InventorySlots.yml`, the server-authoritative resource tiers live in `ResourceMap.yml`, and local auto-managed UI state lives in `Client.yml`.
