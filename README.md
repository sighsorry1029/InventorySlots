...
Use the **Grid / List** button immediately to the left of the search field to switch live. List is the default for new installations; existing saved selections are preserved, and your selection is saved locally between sessions. List shows recipe names on the left and the selected recipe's scrollable details on the right, without a recipe hover popup. Search, filters, favorites, selection, crafting quantity, and queued crafting are shared between views. List scrolling leaves the selected recipe unchanged. Craft, Upgrade, Jewelcrafting's Socket tab, and Recycle N Reclaim's Reclaim tab support both views. In List, Socket risk warnings and Reclaim status/blocking messages appear above the item description and scroll with it. Material costs, returned-item icons, and action buttons remain below; Grid keeps its existing bottom messages. Other external crafting tabs keep their existing layouts.
