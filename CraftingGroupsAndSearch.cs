...
public sealed partial class InventorySlotsPlugin
{
    ...
    private static void UpdateCraftingGroupRail(InventoryGui gui, RectTransform grid)
    {
        ...
        if (!_craftingGroupRailDirty && _craftingGroupRailStamp.Equals(stamp))
        {
            return;
        }
    ...
    private static string GetCraftingClearFavoritesKeyDisplayText() =>
        JoinShortcutDisplayTexts(
            _craftingClearFavoritesKey != null ? _craftingClearFavoritesKey.Value.GetDisplayText() : "Mouse2",
    ...
    private static void UpdateCraftingSearchInput(InventoryGui gui, RectTransform grid)
    {
        ...
        if (!CraftingUi.SearchInputDirty && CraftingUi.SearchInputStamp.Equals(stamp))
        {
            return;
        }
    ...
    private static bool TryGetCraftingTabRowBounds(InventoryGui gui, out CraftingTabRowBounds bounds)
    {
        bounds = default;
        if (gui.m_crafting == null || gui.m_tabCraft == null || gui.m_tabCraft.transform is not RectTransform craftTab)
        {
        ...
        Vector3[] corners = new Vector3[4];
        craftTab.GetWorldCorners(corners);
        Vector3 craftTopLeft = gui.m_crafting.InverseTransformPoint(corners[1]);
        Vector3 craftBottomLeft = gui.m_crafting.InverseTransformPoint(corners[0]);
        float rowCenter = (craftTopLeft.y + craftBottomLeft.y) * 0.5f;
        float rowHeight = Mathf.Max(1f, Mathf.Abs(craftTopLeft.y - craftBottomLeft.y));
        float tolerance = Mathf.Max(rowHeight * 1.25f, 24f);

        float left = float.MaxValue;
        float right = float.MinValue;
        float top = float.MinValue;
        float bottom = float.MaxValue;
        foreach (Button tab in gui.m_crafting.GetComponentsInChildren<Button>(includeInactive: true))
        {
            if (!TryGetCraftingTabButtonBounds(gui, tab, tabRoot, craftTab, rowCenter, rowHeight, tolerance, corners,
                    out Vector3 localBottomLeft, out Vector3 localTopLeft, out Vector3 localTopRight))
            {
                continue;
            }

            left = Mathf.Min(left, localTopLeft.x);
            right = Mathf.Max(right, localTopRight.x);
            top = Mathf.Max(top, localTopLeft.y);
            bottom = Mathf.Min(bottom, localBottomLeft.y);
        }
        ...
        bounds = new CraftingTabRowBounds(left, right, top, bottom);
        return true;
    }

    private static bool TryGetCraftingTabButtonBounds(
        InventoryGui gui,
        Button? tab,
        Transform tabRoot,
        RectTransform craftTab,
        float rowCenter,
        float rowHeight,
        float tolerance,
        Vector3[] corners,
        out Vector3 localBottomLeft,
        out Vector3 localTopLeft,
        out Vector3 localTopRight)
    {
        localBottomLeft = default;
        localTopLeft = default;
        localTopRight = default;
        if (tab == null || IsUnityNull(tab) || !tab.gameObject.activeInHierarchy || tab.transform is not RectTransform tabRect)
        {
            return false;
        }

        bool isVanillaTab = tab == gui.m_tabCraft || tab == gui.m_tabUpgrade;
        bool isSameTabRoot = tab.transform.parent == tabRoot || tab.transform.IsChildOf(tabRoot);
        if (!isVanillaTab && IsOwnedCraftingUiTransform(tab.transform))
        {
            return false;
        }

        if (!isVanillaTab && !isSameTabRoot && !tab.interactable)
        {
            return false;
        }

        if (!isVanillaTab && IsKnownNonTabCraftingButton(gui, tab))
        {
            return false;
        }

        tabRect.GetWorldCorners(corners);
        localBottomLeft = gui.m_crafting.InverseTransformPoint(corners[0]);
        localTopLeft = gui.m_crafting.InverseTransformPoint(corners[1]);
        localTopRight = gui.m_crafting.InverseTransformPoint(corners[2]);
        Vector3 localBottomRight = gui.m_crafting.InverseTransformPoint(corners[3]);
        float tabCenter = (localTopLeft.y + localBottomLeft.y) * 0.5f;
        if (Mathf.Abs(tabCenter - rowCenter) > tolerance)
        {
            return false;
        }
        ...
        if (width < 36f || width > 260f || height < rowHeight * 0.55f || height > rowHeight * 1.75f)
        {
            return false;
        }

        float right = Mathf.Max(localTopRight.x, localBottomRight.x);
        float left = Mathf.Min(localTopLeft.x, localBottomLeft.x);
        return right > left;
    }

    private static bool IsKnownNonTabCraftingButton(InventoryGui gui, Button button)
    {
        Transform transform = button.transform;
        if (gui.m_craftButton != null && !IsUnityNull(gui.m_craftButton) && transform.IsChildOf(gui.m_craftButton.transform))
        {
            return true;
        }

        if (gui.m_repairButton != null && !IsUnityNull(gui.m_repairButton) && transform.IsChildOf(gui.m_repairButton.transform))
        {
            return true;
        }

        if (gui.m_recipeListScroll != null && !IsUnityNull(gui.m_recipeListScroll) && transform.IsChildOf(gui.m_recipeListScroll.transform))
        {
            return true;
        }

        if (_craftingRecipeGrid != null && !IsUnityNull(_craftingRecipeGrid) && transform.IsChildOf(_craftingRecipeGrid))
        {
            return true;
        }

        if (_craftingGroupRail != null && !IsUnityNull(_craftingGroupRail) && transform.IsChildOf(_craftingGroupRail))
        {
            return true;
        }
