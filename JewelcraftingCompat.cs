...
public sealed partial class InventorySlotsPlugin
{
    ...
    private static void SanitizeJewelcraftingElementClone(GameObject? root, bool restoreVanillaEquipped)
    {
        ...
        if (restoreVanillaEquipped)
        {
            RestoreVanillaEquippedOverlay(root.transform);
        }

        foreach (Transform transform in root.GetComponentsInChildren<Transform>(includeInactive: true))
        {
            if (transform == root.transform || !IsJewelcraftingUiDecoration(transform))
            {
                continue;
            }

            transform.gameObject.SetActive(false);
            foreach (Graphic graphic in transform.GetComponentsInChildren<Graphic>(includeInactive: true))
            {
    ...
    private static void RestoreVanillaEquippedOverlay(Transform root)
    {
        Transform? original = root.Find("equiped_jc_disabled");
        if (original == null)
        {
            return;
        }

        Transform? jewelcraftingEquipped = root.Find("equiped");
        if (jewelcraftingEquipped != null && jewelcraftingEquipped != original)
        {
            jewelcraftingEquipped.name = "InventorySlots_JewelcraftingEquippedDisabled";
            jewelcraftingEquipped.gameObject.SetActive(false);
        }

        original.name = "equiped";
        original.gameObject.SetActive(false);
        if (original.TryGetComponent(out Image image))
        {
            image.enabled = true;
    ...
    private static bool IsJewelcraftingUiDecoration(Transform transform)
    {
        string name = transform.name;
        return string.Equals(name, "JC_ItemBackground", StringComparison.Ordinal) ||
               string.Equals(name, "JC_SelectedItem", StringComparison.Ordinal) ||
               string.Equals(name, "InventorySlots_JewelcraftingEquippedDisabled", StringComparison.Ordinal) ||
               string.Equals(name, "TrannyHoles", StringComparison.Ordinal) ||
               string.Equals(name, "Transmute_Press_Interact", StringComparison.Ordinal) ||
               string.Equals(name, "Bkg (1)", StringComparison.Ordinal) ||
               name.StartsWith("Transmute_", StringComparison.Ordinal) ||
               name.IndexOf("Socket", StringComparison.OrdinalIgnoreCase) >= 0 ||
               name.StartsWith("JC_", StringComparison.Ordinal);
    }
