...
상자를 직접 열 때는 한 번에 한 플레이어만 사용하는 바닐라의 사용 중 검사와 소유권 이전을 따름. 범위 작업은 상자를 하나씩 처리하며, 다른 플레이어가 소유한 사용 중이지 않은 상자에는 현재 소유자의 승인을 요청함. 소유권과 저장된 인벤토리 상태가 도착한 것을 확인한 뒤 아이템을 옮김. 거절되거나 응답 시간이 초과된 상자는 아이템을 옮기지 않고 건너뜀. 자신이 이미 열고 있는 상자에도 퀵 스택할 수 있음.

내장 MultiUserChest 구현과 `Enable Multi User Chest` 설정은 제거됨. 기존 설정 항목과 상자에 남은 이전 처리 기록은 사용하거나 삭제하지 않음. 외부 MultiUserChest 모드를 설치하면 동시 열기는 해당 모드가 담당하며, InventorySlots의 범위 작업은 자신이 소유하지 않은 상자를 제외함.
