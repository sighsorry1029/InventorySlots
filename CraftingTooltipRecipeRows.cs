...
{
    private const float CraftingHoverRecipeOverlayBottomPadding = 6f;
    ...
    private static RectTransform? ResolveCraftingHoverTooltipPanel(GameObject tooltip)
    {
        if (tooltip.transform.childCount > 0 && tooltip.transform.GetChild(0) is RectTransform childPanel)
        {
            return childPanel;
        }

        return tooltip.transform as RectTransform;
    }

    private static TMP_Text? FindTooltipText(GameObject tooltip, string name)
    {
        Transform transform = Utils.FindChild(tooltip.transform, name);
    ...
    private sealed class CraftingTooltipNativePanelLayoutState : MonoBehaviour
    {
        public float AppliedOffsetMinYDelta;
    }

    private static void ApplyCraftingTooltipNativePanelLayout(RectTransform tooltipRect, RectTransform panel, float overlayHeight)
    {
        if (CraftingUi.TooltipExpandedPanel != null &&
            !IsUnityNull(CraftingUi.TooltipExpandedPanel) &&
            CraftingUi.TooltipExpandedPanel != panel)
        {
            RestoreCraftingTooltipNativePanelLayout(restoreAnyPanel: true);
        }

        CraftingTooltipNativePanelLayoutState state =
            panel.GetComponent<CraftingTooltipNativePanelLayoutState>() ??
            panel.gameObject.AddComponent<CraftingTooltipNativePanelLayoutState>();
        if (Mathf.Abs(state.AppliedOffsetMinYDelta) > 0.001f)
        {
            panel.offsetMin = new Vector2(panel.offsetMin.x, panel.offsetMin.y - state.AppliedOffsetMinYDelta);
            state.AppliedOffsetMinYDelta = 0f;
        }

        RectTransform? parent = panel.parent as RectTransform;
        float delta;
        if (parent != null &&
            TryGetHoverTooltipTextBounds(tooltipRect, parent, out Rect textBounds) &&
            TryGetRectTransformBounds(panel, parent, out Rect panelBounds))
        {
            float desiredRowTopY = textBounds.yMin - CraftingHoverRecipeOverlayBottomPadding;
            float desiredPanelBottomY = desiredRowTopY - overlayHeight;
            float maxPanelBottomY = panelBounds.yMax - overlayHeight - 1f;
            desiredPanelBottomY = Mathf.Min(desiredPanelBottomY, maxPanelBottomY);
            delta = desiredPanelBottomY - panelBounds.yMin;
        }
        else
        {
            delta = -overlayHeight;
        ...
        panel.offsetMin = new Vector2(panel.offsetMin.x, panel.offsetMin.y + delta);
        state.AppliedOffsetMinYDelta = delta;
        CraftingUi.TooltipExpandedPanel = panel;
    }

    private static void RestoreCraftingTooltipNativePanelLayout(bool restoreAnyPanel)
    {
        RectTransform? panel = CraftingUi.TooltipExpandedPanel;
        if (!restoreAnyPanel && CraftingUi.TooltipRecipeOverlay != null && !IsUnityNull(CraftingUi.TooltipRecipeOverlay))
        {
            panel = CraftingUi.TooltipRecipeOverlay.parent as RectTransform;
        }

        if (panel == null || IsUnityNull(panel))
        {
            CraftingUi.TooltipExpandedPanel = null;
            return;
        }

        CraftingTooltipNativePanelLayoutState? state = panel.GetComponent<CraftingTooltipNativePanelLayoutState>();
        if (state != null && !IsUnityNull(state) && Mathf.Abs(state.AppliedOffsetMinYDelta) > 0.001f)
        {
            panel.offsetMin = new Vector2(panel.offsetMin.x, panel.offsetMin.y - state.AppliedOffsetMinYDelta);
            state.AppliedOffsetMinYDelta = 0f;
        }

        if (CraftingUi.TooltipExpandedPanel == panel)
        {
            CraftingUi.TooltipExpandedPanel = null;
        }
    }
    ...
    private static RectTransform ResolveCraftingTooltipRecipeOverlayParent(RectTransform tooltipRect, RectTransform fallback)
    {
        if (TryGetHoverTooltipAttachmentRect(tooltipRect, out RectTransform attachmentRect))
        {
    ...
    private static void ConfigureCraftingTooltipRecipeOverlayBackground(RectTransform tooltipRect, Image? image)
    {
        if (image == null)
        {
            return;
        ...
        Image? source = FindTooltipBackgroundImageSource(tooltipRect);
        if (source != null && !IsUnityNull(source) && source.sprite != null)
        {
        ...
        else
        {
            ...
            image.material = null;
            image.color = new Color(0f, 0f, 0f, GetCraftingHoverTooltipBackgroundAlpha());
            image.type = Image.Type.Sliced;
            image.fillCenter = true;
        }

        Color color = image.color;
        color.a = GetCraftingHoverTooltipBackgroundAlpha();
        image.color = color;
        image.enabled = true;
    ...
    private static Image? FindTooltipBackgroundImageSource(RectTransform tooltipRect)
    {
        Image? best = null;
        float bestArea = 0f;
        foreach (Image image in tooltipRect.GetComponentsInChildren<Image>(includeInactive: false))
        {
            if (image == null ||
                IsUnityNull(image) ||
                !image.enabled ||
    ...
    private static void PlaceCraftingTooltipRecipeOverlayBelowTooltip(RectTransform tooltipRect, RectTransform overlay, Rect? tooltipBounds)
    {
        ...
        if (tooltipBounds.HasValue)
        {
            Rect bounds = tooltipBounds.Value;
            overlay.position = parent.TransformPoint(new Vector3(bounds.xMin, bounds.yMin, 0f));
            return;
        }

        Vector3[] corners = new Vector3[4];
        tooltipRect.GetWorldCorners(corners);
        Vector3 fallbackBottomLeft = parent.InverseTransformPoint(corners[0]);
        overlay.position = parent.TransformPoint(fallbackBottomLeft);
    }

    private static bool TryGetHoverTooltipPanelBounds(RectTransform tooltipRect, RectTransform parent, out Rect bounds)
    {
        if (TryGetHoverTooltipContentBounds(tooltipRect, parent, out bounds))
        {
            return true;
        }

        return TryGetTooltipVisualBounds(tooltipRect, parent, out bounds);
    }

    private static bool TryGetHoverTooltipContentBounds(RectTransform tooltipRect, RectTransform parent, out Rect bounds)
    {
        bounds = default;
        if (!TryGetHoverTooltipTextBounds(tooltipRect, parent, out Rect textBounds))
        {
            return false;
        }

        if (TryGetHoverTooltipMainPanelBounds(tooltipRect, parent, textBounds, out bounds))
        {
            return true;
        }

        if (TryGetHoverTooltipBackgroundBounds(tooltipRect, parent, textBounds, out bounds))
        {
            return true;
        }

        if (TryGetHoverTooltipTextRectBounds(tooltipRect, parent, textBounds, out Rect textRectBounds))
        {
            const float textRectTopPadding = 18f;
            const float textRectBottomPadding = 16f;
            bounds = Rect.MinMaxRect(
                textRectBounds.xMin,
                textBounds.yMin - textRectBottomPadding,
                textRectBounds.xMax,
                textBounds.yMax + textRectTopPadding);
            return true;
        }

        const float horizontalPadding = 26f;
        const float topPadding = 18f;
        const float bottomPadding = 16f;
        bounds = Rect.MinMaxRect(
            textBounds.xMin - horizontalPadding,
            textBounds.yMin - bottomPadding,
            textBounds.xMax + horizontalPadding,
            textBounds.yMax + topPadding);
        return true;
    }

    private static bool TryGetHoverTooltipTextBounds(RectTransform tooltipRect, RectTransform parent, out Rect bounds)
    {
        bounds = default;
        bool found = false;
        float minX = 0f;
        ...
        foreach (TMP_Text text in tooltipRect.GetComponentsInChildren<TMP_Text>(includeInactive: false))
        {
            if (text == null ||
                IsUnityNull(text) ||
                !text.enabled ||
                ...
                IsOwnedCraftingUiTransform(text.transform))
            {
                continue;
            }
            ...
            if (textBounds.size.sqrMagnitude <= 0.001f)
            {
                continue;
            }
            ...
            if (!float.IsNaN(preferredHeight) && !float.IsInfinity(preferredHeight) && preferredHeight > textBounds.size.y)
            {
                min.y = Mathf.Min(min.y, max.y - preferredHeight);
            }
            ...
            {
                Vector3 local = parent.InverseTransformPoint(text.transform.TransformPoint(corners[i]));
                if (!found)
                {
                    minX = local.x;
                    minY = local.y;
    ...
    private static bool TryGetHoverTooltipMainPanelBounds(RectTransform tooltipRect, RectTransform parent, Rect textBounds, out Rect bounds)
    {
        bounds = default;
        if (tooltipRect.childCount == 0)
        {
            return false;
        }

        Transform child = tooltipRect.GetChild(0);
        RectTransform? panel = child as RectTransform;
        if (panel == null ||
            IsUnityNull(panel) ||
            !panel.gameObject.activeInHierarchy ||
            IsOwnedCraftingUiTransform(panel))
        {
            return false;
        }

        return TryGetRectTransformBounds(panel, parent, out bounds) &&
               IsHoverTooltipPanelBoundsCandidate(bounds, textBounds);
    }

    private static bool TryGetHoverTooltipAttachmentRect(RectTransform tooltipRect, out RectTransform attachmentRect)
    {
        attachmentRect = null!;
        RectTransform parent = tooltipRect.parent as RectTransform ?? tooltipRect;
        if (!TryGetHoverTooltipTextBounds(tooltipRect, parent, out Rect textBounds))
        {
            return TryGetHoverTooltipMainPanelRect(tooltipRect, out attachmentRect);
        }

        if (TryGetHoverTooltipBackgroundRect(tooltipRect, parent, textBounds, out attachmentRect))
        {
            return true;
        }

        if (TryGetHoverTooltipMainPanelRect(tooltipRect, out RectTransform mainPanel) &&
            TryGetRectTransformBounds(mainPanel, parent, out Rect mainPanelBounds) &&
            IsHoverTooltipPanelBoundsCandidate(mainPanelBounds, textBounds))
        {
            attachmentRect = mainPanel;
            return true;
        }
    ...
    private static bool TryGetHoverTooltipMainPanelRect(RectTransform tooltipRect, out RectTransform panel)
    {
        panel = null!;
        if (tooltipRect.childCount == 0)
        {
        ...
        if (tooltipRect.GetChild(0) is not RectTransform childPanel ||
            IsUnityNull(childPanel) ||
            !childPanel.gameObject.activeInHierarchy ||
            IsOwnedCraftingUiTransform(childPanel))
        {
            return false;
        }
    ...
    private static bool TryGetHoverTooltipTextRectBounds(RectTransform tooltipRect, RectTransform parent, Rect textBounds, out Rect bounds)
    {
        bounds = default;
        bool found = false;
        float minX = 0f;
        ...
        foreach (TMP_Text text in tooltipRect.GetComponentsInChildren<TMP_Text>(includeInactive: false))
        {
            ...
            if (rect == null ||
                IsUnityNull(rect) ||
                !TryGetRectTransformBounds(rect, parent, out Rect rectBounds) ||
                rectBounds.width < textBounds.width * 0.65f ||
                rectBounds.height < textBounds.height * 0.35f)
            {
                continue;
            }
            ...
            if (!found)
            {
                minX = rectBounds.xMin;
                minY = rectBounds.yMin;
                maxX = rectBounds.xMax;
                maxY = rectBounds.yMax;
                found = true;
                continue;
            }

            minX = Mathf.Min(minX, rectBounds.xMin);
            minY = Mathf.Min(minY, rectBounds.yMin);
            maxX = Mathf.Max(maxX, rectBounds.xMax);
            maxY = Mathf.Max(maxY, rectBounds.yMax);
        }
    ...
    private static bool TryGetHoverTooltipBackgroundBounds(RectTransform tooltipRect, RectTransform parent, Rect textBounds, out Rect bounds)
    {
        bounds = default;
        bool found = false;
        float bestArea = 0f;
        ...
        foreach (Image image in tooltipRect.GetComponentsInChildren<Image>(includeInactive: false))
        {
            ...
            if (!TryGetRectTransformBounds(image.rectTransform, parent, out Rect imageBounds) ||
                !IsHoverTooltipPanelBoundsCandidate(imageBounds, textBounds) ||
                !imageBounds.Contains(textCenter))
            {
                continue;
            }

            float area = imageBounds.width * imageBounds.height;
    ...
    private static bool TryGetHoverTooltipBackgroundRect(RectTransform tooltipRect, RectTransform parent, Rect textBounds, out RectTransform attachmentRect)
    {
        attachmentRect = null!;
        bool found = false;
        float bestArea = 0f;
        ...
        foreach (Image image in tooltipRect.GetComponentsInChildren<Image>(includeInactive: false))
        {
            ...
            if (!TryGetRectTransformBounds(rect, parent, out Rect imageBounds) ||
                !IsHoverTooltipPanelBoundsCandidate(imageBounds, textBounds) ||
                !imageBounds.Contains(textCenter))
            {
                continue;
    ...
    private static bool IsHoverTooltipPanelBoundsCandidate(Rect bounds, Rect textBounds)
    {
        return bounds.width >= textBounds.width * 0.75f &&
               bounds.height >= textBounds.height * 0.75f &&
               bounds.height <= textBounds.height + 220f &&
               bounds.Overlaps(textBounds, allowInverse: false);
    }

    private static bool TryGetRectTransformBounds(RectTransform rect, RectTransform parent, out Rect bounds)
    {
        bounds = default;
    ...
    private static bool TryGetTooltipVisualBounds(RectTransform tooltipRect, RectTransform parent, out Rect bounds)
    {
        bounds = default;
        bool found = false;
        float minX = 0f;
        ...
        foreach (Graphic graphic in tooltipRect.GetComponentsInChildren<Graphic>(includeInactive: false))
        {
            if (graphic == null ||
                IsUnityNull(graphic) ||
                !graphic.enabled ||
                !graphic.gameObject.activeInHierarchy ||
                IsOwnedCraftingUiTransform(graphic.transform))
            {
                continue;
            }
