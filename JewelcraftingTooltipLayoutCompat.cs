...
public sealed partial class InventorySlotsPlugin
{
    ...
    private static bool ShouldLetNativeJewelcraftingInventoryTooltipOwnHover(UITooltip tooltip)
    {
        ...
        InventoryHoverTooltipItemSource? source = GetInventoryContainerHoverTooltipItemSource(tooltip);
        if (source?.Item?.m_shared == null || !ShouldAttemptJewelcraftingPinnedTooltip(source.Value.Item))
        {
            return false;
        }

        return IsJewelcraftingSocketTooltipSource(tooltip) ||
               TryGetLiveJewelcraftingTooltipSource(tooltip, source.Value.Item, out _);
    }

    private static bool TryGetLiveJewelcraftingTooltipSource(UITooltip? tooltip, ItemData? item, out RectTransform? sourceRoot)
    {
        sourceRoot = null;
        if (!HasJewelcraftingActive ||
            tooltip == null ||
            IsUnityNull(tooltip) ||
            item?.m_shared == null ||
            !IsJewelcraftingSocketTooltipSource(tooltip))
        {
            return false;
        }

        InventoryHoverTooltipItemSource? currentSource = GetInventoryContainerHoverTooltipItemSource(tooltip);
        if (currentSource?.Item?.m_shared != null &&
            !CanUseJewelcraftingTooltipSourceItem(item, currentSource.Value.Item))
        {
            return false;
        }

        GameObject? liveTooltip = UITooltip.m_tooltip;
        Transform? source = liveTooltip != null && !IsUnityNull(liveTooltip)
            ? liveTooltip.transform.Find("Bkg (1)")
            : null;
        sourceRoot = source != null ? source.GetComponent<RectTransform>() : null;
        return sourceRoot != null && !IsUnityNull(sourceRoot);
    }

    private static bool IsJewelcraftingSocketTooltipSource(UITooltip tooltip)
    {
        ...
        if (socketTooltip == null || IsUnityNull(socketTooltip))
        {
            return false;
        }

        if (tooltip.m_tooltipPrefab == socketTooltip)
        {
            return true;
        }

        GameObject? liveTooltip = UITooltip.m_tooltip;
        if (liveTooltip == null || IsUnityNull(liveTooltip))
        {
            return false;
        }

        return string.Equals(Utils.GetPrefabName(liveTooltip), socketTooltip.name, StringComparison.Ordinal);
    }

    private static bool CanUseJewelcraftingTooltipSourceItem(ItemData requested, ItemData source)
    {
        if (ReferenceEquals(requested, source))
        {
            return true;
        }

        return requested.m_shared != null &&
               source.m_shared != null &&
               string.Equals(GetItemPrefabName(requested), GetItemPrefabName(source), StringComparison.Ordinal) &&
               string.Equals(requested.m_shared.m_name, source.m_shared.m_name, StringComparison.Ordinal) &&
               requested.m_variant == source.m_variant &&
               requested.m_quality == source.m_quality &&
               GetItemCustomDataOrderIndependentHash(requested) == GetItemCustomDataOrderIndependentHash(source);
    }
